...
private:
    ...
    std::shared_ptr<wui::scroll> vertScroll = std::make_shared<wui::scroll>(1000, 100, 10.0, wui::orientation::vertical, [this](wui::scroll_state, int32_t v) { welcomeText->set_text(std::to_string(v)); });
    std::shared_ptr<wui::scroll> horScroll = std::make_shared<wui::scroll>(100, 0, 0.1, wui::orientation::horizontal, [this](wui::scroll_state, int32_t v) { welcomeText->set_text(std::to_string(v)); });
};
