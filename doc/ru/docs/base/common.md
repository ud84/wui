...
Контрол принимает `{left, top, right, bottom}`. Для позиции `(10, 50)`, ширины 200 и высоты 30 задайте `{10, 50, 210, 80}`. Значение `{-1, -1, width, height}` в аргументе позиции `window::init()` центрирует отдельное окно. У `graphic::draw_graphic()` есть документированное исключение x/y/width/height.
