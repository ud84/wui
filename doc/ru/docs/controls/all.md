...
| [Text](text.md) | Подпись и выравнивание текста |
| [Scroll](scroll.md) | Отдельная полоса прокрутки |
...
| [Panel](panel.md) | Фон и пользовательская графика; не контейнер |
...
Визуальные контролы реализуют `i_control`. `message` и `tray_icon` — вспомогательные классы с отдельным API:
