...
[[nodiscard]] bool use_ini_file(std::string_view file_name);

#ifdef _WIN32
/// Initializes the application config from a registry
[[nodiscard]] bool use_registry(std::string_view app_key, HKEY root = HKEY_CURRENT_USER);
#endif

/// Creates a config from the registry on windows or ini file on other systems
[[nodiscard]] bool create_config(std::string_view file_name,
