                     ...
                     _In_ int       nCmdShow)
{
	...
	{
		std::shared_ptr<WUI::Button> unplugButton(new WUI::Button("Unplug window", [window, parentWindow]() { window->RemoveControl(parentWindow); parentWindow->Init(WUI::WindowType::Dialog, WUI::Rect{ 50, 50, 250, 250 }, "Parent window unplugged!"); }));
		...
		parentWindow->AddControl(plugButton, WUI::Rect{ 10, 55, 110, 75 });
	}
